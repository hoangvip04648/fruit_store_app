  ...
  const handleOrderButton = async () => {
    ...
    if (products.length == 0) {
      ...
      productBody.forEach(item => {
        item.totalPrice = item.price * item.numberItem;
      });
      await axios
        .post(`${rootUrl}/dat-hang`, {
        ...
        .then(res => {
          if (res.data.success) {
            ...
            if (res.data.success == true) {
              ...
              Alert.alert('Thông báo', 'Đặt hàng không thành công', [
                {text: 'CANCLE'},
                {text: 'OK'},
              ]);
            }
          ...
          AsyncStorage.removeItem('ListProduct')
            .then(value => {
              AsyncStorage.setItem(
                'ListProduct',
                JSON.stringify(ListProductJson),
              );
  ...
  const firstProduct = item => {
    return (
      <View
        ...
        <View style={{flex: 2}}>
          <Text
            style={{fontSize: 16, color: 'rgba(0,0,0,0.7)', marginBottom: 5}}>
            {item.numberItem}x{' '}
        ...
        <View style={{flex: 2}}>
          <Text
            style={{fontSize: 16, color: 'rgba(0,0,0,0.7)', paddingLeft: 6}}>
            {parseMoney(item.price)} đ
          </Text>
        </View>
        <View style={{flex: 1}}>
          <TouchableOpacity onPress={() => removeAProduct(item.name)}>
            <View
              style={{backgroundColor: '#d9534f', width: 34, borderRadius: 4}}>
              <Text
                style={{
    ...
    return (
      <View
        ...
        <View style={{flex: 2}}>
          <Text
            style={{fontSize: 16, color: 'rgba(0,0,0,0.7)', marginBottom: 5}}>
            {item.numberItem}x {item.name}
          </Text>
        </View>
        <View style={{flex: 2}}>
          <Text
            style={{fontSize: 16, color: 'rgba(0,0,0,0.7)', paddingLeft: 6}}>
            {parseMoney(item.price * item.numberItem)} đ
          </Text>
        </View>
        <View style={{flex: 1}}>
          <TouchableOpacity onPress={() => removeAProduct(item.name)}>
            ...
            <View
              ...
              {products.length > 0 &&
                products.map((item, index) => {
                  tongtien += item.price * item.numberItem;
                  giamgia +=
                    ((item.price * item.discount) / 100) * item.numberItem;
                  thanhtien = tongtien - giamgia;
              ...
              {products.length == 0 && <Text>Không có sản phẩm</Text>}
            </View>
            <Text style={{fontSize: 16, fontWeight: 'bold', paddingTop: 10}}>
              Tổng tiền: {parseMoney(tongtien)} đ
            </Text>
            <Text style={{fontSize: 16, fontWeight: 'bold', paddingTop: 10}}>
              Giảm giá : {parseMoney(giamgia)} đ
            </Text>
          </View>
          <View style={{flex: 2}} />
          <View style={{flex: 2}} />
